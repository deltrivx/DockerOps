...
CHANGELOG = [
    {
        ...
        "date": "2026-09-20",
        "items": [
            "设备连接：按「本机 / Docker API / SSH」三选一接入，平台由探测得出而不是手动指定",
            "新增设备并连接合并为一次操作，避免先建后连两步",
            "设备列表展示连接方式、识别到的平台与能力；未识别的主机如实显示，不再默认假设",
            "镜像更新改为按容器单独设置：可选自动 / 提醒 / 忽略，无法原地重建的容器如实标注原因",
            "自动更新带三道熔断：策略非自动的跳过并说明、整轮无合格目标记为未执行、连续失败的容器停用重试",
            "DockerOps 自身容器不参与自动更新，避免更新过程中重启中断当轮",
            "支持手动执行一次自动更新，并清理已不存在的容器策略记录",
        ],
    },
    {
        "version": "0.8.3",
